...
public class Get_By_Valid_Owner_Invalid_PartnerAdd extends TestBase {
	int count = 0;
	JsonPath json;
	...
	public Get_By_Valid_Owner_Invalid_PartnerAdd() {
	...
	public void get_CBRCount() throws Exception {
		test = report.startTest("Get_By_Valid_Owner_Invalid_PartnerAdd");

		Response result = CBR_COUNT_GetCount.CBR_Count_Get("Valid_Owner_Invalid_Partner");
		...
		if (error_message.equals("EDI Address"+" "+address_given+" "+"does not exist")) {
			test.log(LogStatus.PASS, " Response message validation Passed for" + " " + "Valid_Owner_Invalid_Partner");
		} else {
