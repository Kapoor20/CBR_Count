...
public class Get_By_Invalid_OwnerAndPartnerAddress extends TestBase{
	int count = 0;

	CBR_COUNT_GetCount CBR_COUNT_GetCount;
	ResultSet queryresult;

	public Get_By_Invalid_OwnerAndPartnerAddress() {
	...
	public void get_CBRCount() throws Exception {
		test = report.startTest("Get_By_Invalid_OwnerAndPartnerAddress");

		Response result = CBR_COUNT_GetCount.CBR_Count_Get("Invalid_Owner_PartnerAddresses");
		...
		if (error_message.equals("EDI Address"+" "+address_given+" "+"does not exist")) {
			test.log(LogStatus.PASS, " Response message validation Passed for" + " " + "Invalid_Owner_PartnerAddresses");
		} else {
