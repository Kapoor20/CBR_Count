...
public class Get_By_Valid_OwnerAndPartnerID_Invalid_OwnerAddress extends TestBase{
	int count = 0;
	JsonPath json;
	...
	public Get_By_Valid_OwnerAndPartnerID_Invalid_OwnerAddress() {
	...
	public void get_CBRCount() throws Exception {
		test = report.startTest("Get_By_Valid_OwnerAndPartnerID_Invalid_OwnerAddress");
		

		Response result = CBR_COUNT_GetCount.CBR_Count_Get("Valid_OwnerAndPartnerID_Invalid_OwnerAddress");
		...
		if (error_message.equals("EDI Address"+" "+address_given+" "+"does not exist")) {
			test.log(LogStatus.PASS, " Response message validation Passed for" + " " + "Valid_OwnerAndPartnerID_Invalid_OwnerAddress");
		} else {
